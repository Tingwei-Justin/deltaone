...
export default class FarmStore {
    ...
    setFarm(
        ...
        farmDetails: {
            ...
            yearlyAPY: number;
            yieldBreakdown:
                | { dailyYield: number; weeklyYield: number; yearlyYield: number; dailyTradingFees: number }
                | { dailyYield: number; weeklyYield: number; yearlyYield: number; dailyTradingFees: number };
            tulipAPR: number;
            baseTokenTotal?: any;
            quoteTokenTotal?: any;
            ...
            totalVlpShares: any;
            coinInLp: number | undefined;
            pcInLp: number | undefined;
            coinToPcRatio?: number | undefined;
        }
    ) {
    ...
    async setPrice(setFarmStoreInitiated: Dispatch<SetStateAction<boolean>>) {
        ...
        const poolLpTokenAccounts = map(
            FARMS,
            farm => new anchor.web3.PublicKey(getFarmPoolLpTokenAccount(farm.symbol))
        );
        const poolCoinTokenaccounts = map(
            FARMS,
            farm => new anchor.web3.PublicKey(getFarmPoolCoinTokenaccount(farm.symbol))
        );
        const poolPcTokenaccounts = map(
            FARMS,
            farm => new anchor.web3.PublicKey(getFarmPoolPcTokenaccount(farm.symbol))
        );
        ...
        const orcaPoolCoinTokenaccounts = map(
            ORCA_FARMS,
            farm => new anchor.web3.PublicKey(getOrcaFarmPoolCoinTokenaccount(farm.symbol))
        );
        const orcaPoolPcTokenaccounts = map(
            ORCA_FARMS,
            farm => new anchor.web3.PublicKey(getOrcaFarmPoolPcTokenaccount(farm.symbol))
        );
        ...
        tokenSupplyForAllFarms.forEach((tokenSupply, index) => {
            ...
            if (farm.singleStake) {
                ...
                price =
                    coinInLp * Number(getTokenPrice(farm.coin.symbol)) + pcInLp * Number(getTokenPrice(farm.pc.symbol));
            }
            ...
            if (isPoolVersionFourOrFive) {
                ...
                try {
                    // @ts-ignore
                    liquidityInUsd = new anchor.BN(
                        new anchor.BN(decodedPoolLpTokenAccountInfo.amount.toString()) * price
                    ...
                    // @ts-ignore
                    liquidityInUsd =
                        new anchor.BN(decodedPoolLpTokenAccountInfo.amount.toString()).div(
                            new anchor.BN(Math.pow(10, farm.decimals))
                        ) * price;
                    apyA = (100 * rewardPerBlockAmountTotalValue) / liquidityInUsd;
                    apyB = (100 * rewardBPerBlockAmountTotalValue) / liquidityInUsd;
                }
                ...
                const liquidityInUsd = new anchor.BN(decodedPoolLpTokenAccountInfo.amount.toString())
                    .mul(priceBN)
                    .div(new anchor.BN(Math.pow(10, farm.decimals)));
            ...
            // @ts-ignore
            const tvl =
                new anchor.BN(new anchor.BN(totalVaultBalance?.toString()) * price).div(
                    new anchor.BN(Math.pow(10, farm.decimals))
                ) * 1;
            ...
            const mintAddress = farm.symbol === "RAY-SRM-DUAL" ? `${farm.mintAddress}0` : farm.mintAddress;

            const dailyAPR = periodicRate + (farm.disabled ? 0 : dailyTradingFees);
            const weeklyAPY =
                100 * getAPY(periodicRate / 24, NUMBER_OF_PERIODS_IN_A_WEEK) +
                (farm.disabled ? 0 : dailyTradingFees * 7);
            const yearlyAPY =
                100 * getAPY(periodicRate / 24, NUMBER_OF_PERIODS_IN_A_YEAR) +
                (farm.disabled ? 0 : Number(tradingFees));
        ...
        ORCA_FARMS.forEach((farm, index) => {
            ...
            const uiAmount = decodedTokenData.supply / Math.pow(10, decodedTokenData.decimals);
            const decodedVaultAccountData = orcaVaultProgram.coder.accounts.decode(
                "Vault",
                Buffer.from(orcaVaultAccountsInfo[index].account.data)
            );
            const { totalVaultBalance, totalVlpShares } = decodedVaultAccountData;
            const totalVaultBalanceInNumber = new anchor.BN(totalVaultBalance?.toString()).div(
                new anchor.BN(Math.pow(10, farm.decimals))
            );
            ...
            let price, poolCoinAmount, poolPCAmount, coinInLp, pcInLp, totalLiquidity, coinToPcRatio;
            ...
            const periodicRate = getOrcaPeriodRate(decodedGlobalFarm, totalLiquidity, farm.reward.decimals, orcaPrice);
